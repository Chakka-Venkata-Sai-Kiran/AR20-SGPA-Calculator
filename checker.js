...
function checkdepartment(){
    ...
    // ckearing the sem value after department is changed
    sem.selectedIndex = 0;
}
...
function checksemester(){
    var table = document.getElementById("myTable");
    var n = table.rows.length - 2;
    // deleting all rows except the last & first two rows
    while(n > 1){
        table.deleteRow(2);
        n=n-1;
    }
    // dictionary of first year subjects
    ...
    // dictionary of sem subjects for all departments
    ...
    for(var i = 0;i < no_sem_subjects; i++){
        ...
        var temp="<b>";
        temp=temp.concat(sem_subjects[i],"</b>");
...
function calculate(){
    ...
    for(var i=0; i< no_sem_subjects;i++){
        ...
        // getting the input field labels
        var temp = "s";
        temp = temp.concat(i);
